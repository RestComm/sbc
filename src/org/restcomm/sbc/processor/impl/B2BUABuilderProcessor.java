 ...
 *
 */
public class B2BUABuilderProcessor implements Processor {
	
	private static transient Logger LOG = Logger.getLogger(B2BUABuilderProcessor.class);

	
	
	public B2BUABuilderProcessor(ProcessorChain callback) {
	...
	private SipServletRequest processRequest(SipServletRequest dmzRequest) {
		...
		SipURI sipUri = Cache.getSipFactory().createSipURI(user, Cache.getRouteMZIPAddress());
		sipUri.setTransportParam(Cache.getRouteMZTransport());
		
		sipUri.setPort(Cache.getRouteMZPort());
