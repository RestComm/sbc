...
public abstract class WhiteListsEndpoint extends SecuredEndpoint {
    ...
    public void init() {
        ...
        BanListConverter converter = new BanListConverter(Type.WHITE, configuration);
    ...
    protected Response getBanList(final String ipAddress, final MediaType responseType) {
        ...
        final BanListDao dao = daos.getWhiteListDao();
    ...
    protected Response updateBanList(final String ipAddress) {
        ...
        WhiteListDao dao = daos.getWhiteListDao();
        ...
        banList=banList.setAction(BanList.Action.REMOVE);
    ...
    protected Response getBanLists(UriInfo info, MediaType responseType) {
        ...
        BanListDao dao = daos.getWhiteListDao();

        BanListFilter filterForTotal;
        try {

            if (localInstanceOnly) {
                filterForTotal = new BanListFilter(Type.WHITE.toString(), ipAddress, accountSid, dateCreated, dateExpires, reason, action, limit, offset, null);
            } else {
                filterForTotal = new BanListFilter(Type.WHITE.toString(), ipAddress, accountSid, dateCreated, dateExpires, reason, action, limit, offset, instanceId);
            }
        ...
        try {
            if (localInstanceOnly) {
                filter = new BanListFilter(Type.WHITE.toString(), ipAddress, accountSid, dateCreated, dateExpires, reason, action, limit, offset, null);
            } else {
                filter = new BanListFilter(Type.WHITE.toString(), ipAddress, accountSid, dateCreated, dateExpires, reason, action, limit, offset, instanceId);
            }
        } catch (ParseException e) {
            return status(BAD_REQUEST).build();
        }
    ...
    private BanList createFrom(final String accountSid, final MultivaluedMap<String, String> data) {
        ...
        builder.setAction(BanList.Action.APPLY);
